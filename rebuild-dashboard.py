...
<div class="sub">TOTAL_JOBS aligned from TOTAL_SCANNED scanned · DIRECT_COUNT direct links · Updated UPDATED_AT</div>
</div>
<div class="sts">
...
function ok(u) {
  ...
  return u.includes('viewjob') || u.includes('greenhouse.io/') || u.includes('lever.co/') || 
         u.includes('ashbyhq.com/') || u.includes('linkedin.com/jobs/view/') ||
         u.includes('workday.com/') || u.includes('myworkdayjobs.com/') ||
         (u.includes('/jobs/') && !u.endsWith('/jobs/') && !u.endsWith('/jobs'));
}
function render(f) {
...
with open('dashboard.html', 'w') as f:
    f.write(html)
print(f'Dashboard: {total} jobs, {direct_count} direct, {os.path.getsize("dashboard.html")//1024}KB')

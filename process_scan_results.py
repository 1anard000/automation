#!/usr/bin/env python3
"""Process greenhouse scan results and add new jobs to database."""
import json
import os
...
TARGET_LOCATIONS_FULL = ['shenzhen', 'hong kong', 'hongkong', 'hk', 'guangzhou', 'shanghai', 'singapore']

# Exclude patterns (Director/VP)
EXCLUDE_PATTERNS = ['director', 'vp ', 'vice president', 'chief', 'c-level', 'intern']

# Senior/Manager level patterns
SENIOR_PATTERNS = ['senior', 'manager', 'lead', 'principal', 'head of', 'gm', 'general manager']

def is_target_location(loc):
    if not loc:
        return False
    loc_lower = loc.lower()
    return any(t in loc_lower for t in TARGET_LOCATIONS_FULL)
...
def is_senior_level(title):
    ...
    for ex in EXCLUDE_PATTERNS:
        if ex in title_lower:
            return False
    # Must be senior/manager level
    return any(p in title_lower for p in SENIOR_PATTERNS)
...
for job in scan_results:
    ...
    # Skip if not senior level
    if not is_senior_level(title):
        continue

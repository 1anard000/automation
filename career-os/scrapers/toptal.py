#!/usr/bin/env python3
"""
Toptal talent network opportunity scraper via web_search.

Scrapes senior product roles from Toptal's talent network and job listings.

Usage (agent-driven):
  1. Run web_search queries (QUERIES below)
  2. Save results to toptal-websearch.json
  3. Run this script to generate toptal-results.json
"""
import json, os, re, sys
from datetime import datetime

OUTPUT = os.path.join(os.path.dirname(__file__), "toptal-results.json")

QUERIES = [
    'site:toptal.com "senior product manager" remote',
    'site:toptal.com "head of product" remote',
    'site:toptal.com "product director" remote',
    'site:toptal.com "VP product" remote',
    'toptal.com senior product manager remote jobs',
    'toptal.com product director APAC remote',
    'toptal.com "product manager" Singapore OR "Hong Kong"',
    'toptal remote product leadership roles Asia',
]
...
def process_results(results):
    ...
    for r in results:
        ...
        if not url or not title:
            continue
        if "toptal.com" not in url.lower():
            continue
        if url in seen_urls:
            continue
        ...
        # Clean title
        clean_title = re.sub(r'\s*\|\s*Toptal.*$', '', title).strip()
        clean_title = re.sub(r'\s*-\s*Toptal.*$', '', clean_title).strip()
...
def main():
    standalone_path = os.path.join(os.path.dirname(__file__), "toptal-websearch.json")
    results = []
    ...
    print(f"\nToptal: {len(jobs)} jobs extracted")

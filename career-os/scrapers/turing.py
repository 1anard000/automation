#!/usr/bin/env python3
"""
Turing.com job scraper via web_search.

Scrapes remote senior product roles from Turing.com talent network.

Usage (agent-driven):
  1. Run web_search queries (QUERIES below)
  2. Save results to turing-websearch.json
  3. Run this script to generate turing-results.json
"""
import json, os, re, sys
from datetime import datetime

OUTPUT = os.path.join(os.path.dirname(__file__), "turing-results.json")

QUERIES = [
    'site:turing.com "senior product manager" remote',
    'site:turing.com "head of product" remote',
    'site:turing.com "VP product" remote',
    'site:turing.com "director product" remote',
    'site:turing.com "product manager" Singapore OR "Hong Kong"',
    'site:turing.com "product" senior APAC',
    'turing.com remote senior product manager jobs',
    'turing.com remote product director jobs APAC',
]
...
def is_relevant(title, snippet):
    ...
    # Must also have seniority signal
...
def process_results(results):
    ...
    for r in results:
        ...
        if not url or not title:
            continue
        if "turing.com" not in url.lower():
            continue
        if url in seen_urls:
            continue
        ...
        # Clean title
        clean_title = re.sub(r'\s*\|\s*Turing.*$', '', title).strip()
        clean_title = re.sub(r'\s*-\s*Turing.*$', '', clean_title).strip()
...
def main():
    standalone_path = os.path.join(os.path.dirname(__file__), "turing-websearch.json")
    results = []
    ...
    print(f"\nTuring: {len(jobs)} jobs extracted")

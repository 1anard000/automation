...
# Crypto companies to exclude
crypto_companies = ['binance', 'okx', 'coins.ph', 'coinbase', 'bybit', 'huobi', 'kucoin', 'bitget', 'gate', 'gate.io']
...
def classify(j):
    ...
    if cat in ['strategy', 'strategy_ops']:
        return 'strategy_bizops'
    if any(kw in title for kw in ['strategy', 'chief of staff', 'chief operating']):
        return 'strategy_bizops'
    if 'business strategy' in title:
        return 'strategy_bizops'
    if 'bizops' in title:
        return 'strategy_bizops'
    ...
    if cat in ['growth', 'gm', 'general_manager', 'growth_expansion', 'general manager']:
        return 'growth_gm'
    if any(kw in title for kw in ['expansion', 'general manager', 'country manager']):
        return 'growth_gm'
    # Only match 'growth' if NOT engineering
    if 'growth' in title and 'engineer' not in title:
        return 'growth_gm'
    if 'head of' in title:
        return 'growth_gm'
    ...
    if cat in ['ops', 'operations', 'business operations']:
        return 'cross_border'
    if any(kw in title for kw in ['operations', 'director', 'regional sales']):
        return 'cross_border'
...
# Fill remaining with best from 'other'

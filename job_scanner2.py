...
SKIP_KEYWORDS = ['intern', 'internship', 'junior']

# Try different Greenhouse company slugs
greenhouse_companies = [
    'okx', 'okxglobal', 'okx-com',
    'stripe', 'stripeinc',
    'airwallex',
    'coupang',
    'bytedance', 'bytedance-limited',
    'figma',
    'affirm',
    'bybit',
]
...
for company in greenhouse_companies:
    ...
    for api_url in [
        f"https://boards-api.greenhouse.io/v1/jobs/{company}",
        f"https://boards-api.greenhouse.io/v1.1/boards/{company}/jobs",
    ]:
        print(f"\n--- Trying {company} at {api_url} ---")
        try:
            ...
            jobs = data.get('jobs', [])
            if not jobs:
                print(f"  No jobs found, trying next URL")
                continue
            ...
            break  # Found jobs, no need to try next URL
